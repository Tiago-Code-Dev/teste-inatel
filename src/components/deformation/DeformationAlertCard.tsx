...
import { AlertTriangle, CircleDot, Thermometer, Gauge, Wrench, Eye } from 'lucide-react';
import { cn } from '@/lib/utils';
import { DeformationAlert } from '@/hooks/useTireDeformation';
import { formatDistanceToNow } from 'date-fns';
import { ptBR } from 'date-fns/locale';

interface DeformationAlertCardProps {
  alert: DeformationAlert;
  onViewTire?: () => void;
  onFix?: () => void;
}

export function DeformationAlertCard({ alert, onViewTire, onFix }: DeformationAlertCardProps) {
  ...
  const typeConfig = {
    deformation: { label: 'Deformação', icon: CircleDot },
    pressure_impact: { label: 'Impacto de Pressão', icon: Gauge },
    temperature_impact: { label: 'Impacto de Temperatura', icon: Thermometer },
    structural: { label: 'Estrutural', icon: AlertTriangle },
  };
    ...
    <Card className={cn('border-l-4', severity.borderColor)}>
      <CardContent className="p-4">
        ...
        <div className="flex items-center gap-2 text-xs text-muted-foreground mb-3">
          ...
          <span>•</span>
          <span>{alert.deformationLevel.toFixed(1)}% deformação</span>
        </div>
